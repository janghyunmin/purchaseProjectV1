...
@AndroidEntryPoint
open class DisclosureInvestmentFragment : BaseFragment<FragmentDisclosureBinding>(FragmentDisclosureBinding::inflate) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.apply {
            investFragment = this@DisclosureInvestmentFragment
            viewModel = viewModel

            arguments?.let { bundle ->
                val data = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                    bundle.getParcelableArrayList("data", InvestmentDisclosureItemVo::class.java)
                } else {
                ...
                when (bundle.getString("searchViewStatus", "N")) {
                    ...
                    "N" -> {
                        ...
                        if (data?.isEmpty() == true) {
                            ...
                            data?.let {
                                val investmentRvAdapter = InvestmentRvAdapter(requireContext())
                                adapterOnClick(investmentRvAdapter, data)
                                disclosureAllRv.run {
                                    adapter = investmentRvAdapter
                                        ...
                                        override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                                            ...
                                            if (!disclosureAllRv.canScrollVertically(1)) {
                                                if (rvPosition == totalCount) {
                                                    page++
                                                    loadMore(investmentRvAdapter, data, "", page)
                                                }
                                            } else {
                                                return
                                            }
                                ...
                                investmentRvAdapter.submitList(data)
                            } ?: run {
                                binding.emptyGroup.visibility = View.VISIBLE
                                binding.disclosureAllRv.visibility = View.GONE
                                binding.goBackTv.visibility = View.GONE
                                    ...
                                    addOnScrollListener(object : RecyclerView.OnScrollListener() {
                                        override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                                            ...
                                            if (!disclosureAllRv.canScrollVertically(1)) {
                                                if (rvPosition == totalCount) {
                                                    page++
                                                    loadMore(investmentRvAdapter, data, searchText, page)
                                                }
                                            } else {
                                                return
                                            }
    ...
    private fun loadMore(rvAdapter: InvestmentRvAdapter, data: List<InvestmentDisclosureItemVo>, keyword: String, page: Int) {
        ...
        moreJob = viewModel.viewModelScope.launch {
            if (keyword.isEmpty()) {
                this@DisclosureInvestmentFragment.viewModel.getBoard("투자공시", portfolioId, "", page, page)
            } else {
                this@DisclosureInvestmentFragment.viewModel.getBoard("투자공시", portfolioId, keyword, page, page)
            }
            ...
            viewModel.disclosureList.collect { vo ->
                when (vo) {
                    is DisclosureTabViewModel.DisclosureListState.Success -> {
                        ...
                        vo.disclosure.investmentDisclosure.disclosure.forEach { newItem ->
    ...
    private fun adapterOnClick(rvAdapter: InvestmentRvAdapter, data: List<InvestmentDisclosureItemVo>) {
        rvAdapter.setItemClickListener(object : InvestmentRvAdapter.OnItemClickListener {
    ...
    companion object {
        fun newInstance(data: List<InvestmentDisclosureItemVo>, portfolioId: String, searchViewStatus: String, searchText: String): DisclosureInvestmentFragment {
            var fragment = DisclosureInvestmentFragment()
